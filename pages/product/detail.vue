	...
	<view class="page">
		<block v-if="(isdata)">
			...
			<scroll-view class='u-pa1 fix-titlenview-trans' scroll-y="true" :scroll-top="scposition" @scroll="scrolltoupper">
				...
				<view class="m-cells" style="margin-top:0upx" v-if="(ProductInfo.IsShowCountdown)">
							...
							<label class="m-imfo-sp-selext m-panel-sp-labellist">
								<block v-for="(items, i) in ProductInfo.SpecLst"  :key="i">
									...
									<block v-for="(item, ii) in items.svLst"  :key="i + '_' + ii">
										<label v-if="(item.IsChecked)" class="m-panel-sp-sellabel" style="margin: 0;margin-left:10rpx;padding: 0 8upx;">{{item.Name}}</label>
									</block>
									<!-- #endif -->
									<!-- #ifndef H5 -->
				...
				<view class="m-cells" style="margin-top:0upx" v-if="(ProductInfo.IsShowCountdown==false)">
					<view class="m-cell m-info-box">
						<view class="m-cell-bd">
							<text>{{ProductInfo.ProductName}}</text>
							<label>{{ProductInfo.ProductTips || " "}}</label>
							<view class="m-info-price">
								...
								<block v-if="ProductInfo.item_unit_points">
									<text style="margin-left: 20upx;">+ <text style="font-size:32upx">{{ProductInfo.item_unit_points}}</text>{{__('积分')}}</text>
								</block>

								<block v-if="ProductInfo.item_unit_sp">
									<text style="margin-left: 20upx;">+ <text style="font-size:32upx">{{ProductInfo.item_unit_sp}}</text>{{__('众宝')}}</text>
								</block>

								<view class="tag-view" v-if="ProductInfo.item_rebate > 0">
									<uni-tag :text="sprintf(__('返利￥%.2f'), number_format(ProductInfo.item_rebate, 2))" type="shopsuite" size="small" inverted="1" circle="true" data-type="groupbuy" style="height: 40upx;line-height: 40upx;"></uni-tag>
								</view>

								<view class="tag-view" v-if="ProductInfo.item_fx_cps > 0">
									<uni-tag :text="sprintf(__('佣金￥%.2f'), number_format(ProductInfo.item_fx_cps, 2))" type="shopsuite" size="small" inverted="1" circle="true" data-type="groupbuy" style="height: 40upx;line-height: 40upx;"></uni-tag>
								</view>
								...
								<view class="progressBarBox" v-if="(ProductInfo.activityType=='GROUPBOOKING' && is_join)">
					...
					<view class="m-cell m-cell-access m-info-sp" v-if="(ProductInfo.activity_item_row.activity_type_id == StateCode.ACTIVITY_TYPE_ONE_PIECE_DISCOUNT)">
						<view class="m-cell-bd">
							...
							<label class="m-imfo-sp-selext"  style="color:#0a0322;font-weight:800;font-size:20upx">
					...
					<view v-if="ProductInfo.reduction_again !== undefined && ProductInfo.reduction_again.activity_type_id == StateCode.ACTIVITY_TYPE_REDUCTION_AGAIN" class="m-cell m-cell-access m-info-sp" @click="showActivityDetail(1140)">
						<view class="m-cell-bd" style="color:red">
							<text class="m-info-tip">{{__('折上折')}}</text>
							<label class="m-imfo-sp-selext">
								  {{ProductInfo.reduction_again.activity_name}}
							</label>
						</view>
						<view class="m-cell-ft">{{__('查看详情')}}</view>
					</view>
					...
					<view class="m-policy"  v-if="(wholesale_policy_rows.length > 0)">
						<view class="m-cell-bd" style="padding:20upx 30upx;display:flex;flex-wrap: nowrap;">
						...
						<view class="m-cell-bd" style="padding:20upx 30upx;display:flex;flex-wrap: nowrap;" v-for="(row,ii) in wholesale_policy_rows" :key="ii">
				...
				<!-- 优惠券展示 -->
					...
					<view class="m-coupon-list1 m-cell m-cell-access m-info-sp "  v-if="(VoucherList.length>0)"  @click="showCouponDetail()">
						<view class="m-cell-bd coupon">
							<text class="m-info-tip">{{__('优惠券')}}</text>
							<view>
								<view class="m-coupon-item1" v-for="(item, ii) in VoucherList" :key="ii">
									<view class="m-coupon-info">
										<view class="m-coupon-text">
											...
											<text>
												{{sprintf(__('满￥%s可用，消耗%s积分领取。'), item.activity_rule.requirement.buy.subtotal, item.activity_rule.requirement.points.needed)}}{{item.activity_remark}}
											</text>
											<!-- <text>
					...
					<view class="m-coupon-list"  v-if="(VoucherList.length>0)" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;height:700upx;display: flex;flex-direction: column;padding：30upx">
						<scroll-view scroll-y style="height: calc(100vh)">
						...
						<button class="u-btn u-btn-default" style="margin-top:20upx;margin:0;padding:0" @click="getCoupon">{{__('一键领取')}}</button>
					</view>
				</uni-popup>
				...
				<view class="m-panel m-panel-access m-comment m-fg-comment" v-if="(ProductInfo.activityType=='GROUPBOOKING' && GB_rows.length>0)">
					...
					<view class="m-panel-bd" :style="(!isAll?'max-height:382upx;overflow:hidden;':'')">
						<view @click="groupBuying" v-for="(item, ii) in GB_rows"  :key="ii" :data-gb_id="(item.gb_id)"
						 class="m-media-box m-media-box-appmsg">
							...
							<view class="m-media-box-bd">
								<button class="u-btn u-btn-default" style="margin-top:20upx">{{__('立即参团')}}</button>
							</view>
						</view>
					</view>
					...
					<view class="evaluate-top">
						...
						<navigator :url="'/pages/product/commentlist?id=' + (proId)" class="evaluate-right">
							<label class="right">{{__('全部评价')}}</label>
								<image style="width:16upx;height:28upx;position: absolute;margin-left:20upx;margin-top:10upx;" src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/goto.png" />
						</navigator>
					</view>
				...
				<view class="m-store" v-if="(Config.MULTISHOP_ENABLE && !if_saas_status && !isStoreFlag)">
					<view class="m-store-info">
						...
						<label style="height:29upx;font-size:30upx;font-family:Alibaba PuHuiTi;font-weight:500;color:rgba(51,51,51,1);position: absolute;margin-top:14upx;margin-left:139upx">{{store_info.store_name}}</label>
						<label style="width:200upx;height:23upx;font-size:24upx;font-family:Alibaba PuHuiTi;font-weight:400;color:rgba(102,102,102,1);position: absolute;margin-top:60upx;margin-left:139upx">{{sprintf(__('在售商品%s件'), store_info.store_product_num)}}</label>
						<navigator :url="'/pagesub/index/store' + '?store_id=' + (ProductInfo.store_id)" hover-class="none" class="evaluate-right" style="position: absolute;margin-top:32upx;margin-left:589upx">
							<label class="right">{{__('进店看看')}}</label>
								<image style="width:16upx;height:28upx;position: absolute;margin-left:20upx;margin-top:10upx;" src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/goto.png" />
						</navigator>
					</view>
					<view v-if="store_info.store_o2o_flag==1 && store_info.store_latitude && store_info.store_longitude" style="height: 100rpx;">
						<view style="padding: 0 30rpx;">
							<view class="m-cell-bd" style="width: 100%;">
								<text class="m-info-tip" style="margin-right: 10rpx;">{{__('营业时间')}} : </text>
								<text class="m-info-tip">{{store_info.store_opening_hours || '00:00'}} - {{store_info.store_close_hours || '00:00'}}</text>
							</view>
						</view>
						<view style="padding: 0 30rpx;">
							<view class="m-cell-bd"  @click.stop="gotomap"  :data-address="(store_info.store_address)"
								:data-name="(store_info.store_name)" :data-lat="(store_info.store_latitude)" :data-lng="(store_info.store_longitude)">
								<view class="m-info-tip location-addr" style="margin-right: 10rpx;width: 400rpx;float:left;">{{store_info.store_address}}</view>
								
								<label style="display: flex;float:right;">
									<image style="width:20upx;height:26upx;margin-top:10rpx;" src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/location.png'></image>
									<label class="u-font" style="line-height: 1.8;">离您 {{store_info.distance}}</label>
								</label>
																		
							</view>
						</view>
					</view>
					<scroll-view class="scroll-view_H" scroll-x="true" style="height:390upx" >
						<navigator :url="(store_info.store_type==3?'/pages/product/details':'/pages/product/detail') + '?pid=' + (item.item_id)"  v-for="(item,i) in store_product_list" :key="i" hover-class="none" class="scroll-view-item_H" style="width:210upx;height:340upx;margin-left:30upx;margin-top:20upx;">
				...
				<view class="m-panel m-panel-access goods-detail-store" v-if="(chain_rows.length>0)">
					<block v-if="(chain_rows.length>0)">
						...
						<view class="m-panel-bd" style="height: 200upx;">
									...
									<view class='item-box'>
										<view class='flex-box'>
											...
											<view class='flex-postion'>
												<image src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/location.png'></image><label>{{item.distance}}</label>
											</view>
										</view>
										<view class='flex-info'>
											...
											<view class='flex-info-box' @click.stop="gotomap" :data-address="(item.chain_district_info)" :data-name="(item.chain_name)" :data-lat="(item.chain_lat)" :data-lng="(item.chain_lng)"><label class='flex-info-title'>{{__('商家地址：')}}</label><label>{{item.chain_district_info}}</label></view>
											<image src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/right.png'></image>
										</view>
									</view>
								</view>
							</scroll-view>
				...
				<view class="m-panel m-panel-access goods-detail-store" v-if="(Config.MULTISHOP_ENABLE && !if_saas_status && !isStoreFlag) && false">
					<navigator redirect="true" :url="'/pagesub/index/store?store_id=' + (ProductInfo.store_id)">
						<view class="m-panel-hd store_name" style="height: 44upx;">
						...
						<view class="m-panel-bd" style="clear: both;"> <!-- v-if="(!store_info.store_is_selfsupport)"  -->
							<view class="store-rate">
								<label class="equal" style="width: 33%;">{{__('描述相符')}}
									<view style="padding-left: 10upx;">{{store_info.store_credit.store_desccredit.credit}}</view>
									<!--<i></i>-->
								</label>
								<label class="equal" style="width: 33%;text-align: center;">{{__('服务态度')}}
									<view style="padding-left: 10upx;">{{store_info.store_credit.store_servicecredit.credit || '--'}}</view>
									<!--<i></i>-->
								</label>
								<label class="equal" style="width: 33%;text-align: right;">{{__('发货速度')}}
									<view style="padding-left: 10upx;">{{store_info.store_credit.store_deliverycredit.credit || '--'}}</view>
									<!--<i></i>-->
								</label>
							</view>
				...
				<view :class="'m-panel m-panel-access m-comment'" v-if="(faq_rows.length>0)" :data-product_id="proId" @click="listFaq">
					...
					<view class="m-panel-bd">
						<view class="m-media-box m-media-box-appmsg" style="padding-top: 20upx;padding-bottom: 20upx;"  v-for="(faq_row, idx) in faq_rows" :key="idx">
							<view class="m-media-box-bd">
								<view class="m-media-box-desc">
									<label style="padding:6upx;font-size:20upx;background-color: #ff6700;color: #fff;">
										{{__('问')}}
									</label>
									<label style="margin-left:10upx;font-size:20upx;">
										{{faq_row.story_title}}
									</label>
				...
				<view :class="'m-panel m-panel-access m-comment'">
					<view class="m-panel-bd">
						<view class="m-media-box m-media-box-appmsg" style="padding-top: 16upx;padding-bottom: 16upx;">
							<view class="m-media-box-bd">
								<view class="m-media-box-desc">
										<label style="padding:6upx;font-size:20upx;background-color: #ff6700;color: #fff;">
											{{__('问大家')}}
				...
				<view>
					<view class="m-panel m-panel-access m-detail">
						...
						<view v-if="(ProductInfo.activityType=='GROUPBOOKING')" :class="'m-Packinglist ' + (tapindex==4?'':'hide')" style="font-size:24upx;">
							<view style="text-align:left;line-height:40upx;">{{__('拼团规则')}}</view>
							<view style="text-align:left;line-height:40upx;">
								{{__('活动时间：')}}{{ProductInfo.activity_item_row.activity_item_starttime}} - {{ProductInfo.activity_item_row.activity_item_endtime}}
							</view>
							<view style="text-align:left;line-height:40upx;">
								{{sprintf(__('支付开团，需在%s天内，成功邀请%s人参团，人数不足自动退款。'), ProductInfo.DayLimit, ProductInfo.UserLimit)}}
							</view>
							<view style="text-align:left;line-height:40upx;">{{__('拼团商品会尽快发货。')}}</view>
							<view style="text-align:left;line-height:40upx;" class="highlight">
								<view>{{__('① 选择商品')}}</view>
								<view>{{__('② 支付开团或参团')}}</view>
								<view>{{__('③ 等待好友参团支付')}}</view>
								<view>{{__('④ 达到人数拼团成功')}}</view>
							</view>
							<view style="text-align:left;line-height:40upx;">{{__('如在限定时间内参团人数没达成，系统自动退款。')}}</view>
						</view>
					...
					<view class="m-footer-btn-list">
						...
						<view data-url="/pages/cart/cart" class="m-footer-btn-item" @click="goTabBar">
							<view class="m-footer-btn-icon">
								...
								<uni-badge v-if="cartNum>0" :text="cartNum + ''" type="danger" style="margin-top: -12rpx;margin-left: 7rpx;"></uni-badge>
							</view>
							<text>{{__('购物车')}}</text>
						</view>
					</view>
			...
			<view :class="'m-panel-sp ' + (selectsp==0?'hide':'') " @click.stop="closesp">
				<view :class="'m-panel-sp-content ' + (selectct==0?'bounceOutDown animated':'bounceInUp animated')" @click.stop >
					<view class="m-panel-sp-pinfo">
						...
						<view class="m-panel-sp-pname">
							...
							<view class="m-pprice" v-if="(ProductInfo.activityType!='GROUPBOOKING')">
								...
								<block v-if="ProductInfo.item_unit_points">
									<text style="margin-left: 20upx;font-size: 22upx;">+ <text style="font-size:26upx">{{ProductInfo.item_unit_points}}</text>{{__('积分')}}</text>
								</block>

								<block v-if="ProductInfo.item_unit_sp">
									<text style="margin-left: 20upx;font-size: 22upx;">+ <text style="font-size:26upx">{{ProductInfo.item_unit_sp}}</text>{{__('众宝')}}</text>
								</block>
							...
							<view v-if="(ProductInfo.SpecLst.length)" class="m-pstock">{{__('已选择：')}}
								<block v-for="(items, i) in ProductInfo.SpecLst"  :key="i">
									<!-- #ifdef H5 -->
									<block v-for="(item, ii) in items.svLst"  :key="i + '_' + ii">
										<span v-if="(item.IsChecked)" style="margin-left: 4upx;">{{item.Name}}</span>
									</block>
									<!-- #endif -->
									<!-- #ifndef H5 -->
									<block v-for="(item, ii) in items.svLst"  :key="ii">
						...
						<block v-if="b2bFlag && ProductInfo.product_spec.length>0">
							<view class="m-panel-sp-listbox-item">
								...
								<block v-for="(item, ii) in product_uniqid" :key="ii">
									<label :class="item[6] == color ? 'm-panel-sp-sellabel m-ys' : 'm-panel-sp-sellabel1 m-ys'" style="width:94%;float:left;height:50upx;line-height:50upx;overflow:hidden;padding-left:10upx;">
										{{ item[4] }}
										<view class="u-cart-num" style="margin-top:-6upx;float:right">
					...
					<view class="m-m-panel-sp-btn">
						<view class="m-pprice" style="width:50%;float:left;height:100upx;line-height:100upx;text-align:center" v-if="b2bFlag && ProductInfo.product_spec.length>0">
						...
						<view class="m-m-panel-sp-lbtn" :style="(ProductInfo.activityType=='GROUPBOOKING'&&isModified==false?'display:none':'')" v-else>
							<view class="u-cart-num" style="margin-top:21upx">
		...
		<uni-popup ref="popup" type="center">
			...
			<view class="uni-banner giftbag" v-if="ProductInfo.giftbag_item_data" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;width: 680upx;height:700upx;padding:26upx;">
				<label class="activity-title">{{ProductInfo.activity_item_row.activity_name}}</label>
				<scroll-view  class="scroll-view_H" scroll-x="true">
					<view class="scroll-view-item_H" v-for="(g_b_item,i) in ProductInfo.giftbag_item_data">
						<view>
							<view hover-class="none" :data-pid="(g_b_item.item_id)" :data-state="(g_b_item.product_state_id)">
								...
								<view class="product-l-img">
									<image style="width:50px;height:50px" :src="g_b_item.product_image" />
								</view>
							</view>
						</view>
					</view>
				...
				<view class="gift-rule">
					{{__('套餐内包含以上商品，原价共')}}<span style="color: red;font-size: 16px;font-weight: bold;">{{ProductInfo.activity_old_price}}</span>{{__('元，组合购买价')}}<span style="color: red;font-size: 16px;font-weight: bold;">{{ProductInfo.activity_item_row.activity_rule.giftbag_amount}}</span>{{__('元，可省去')}}<span style="color: red;font-size: 16px;font-weight: bold;">{{ProductInfo.activity_diff_price}}</span>{{__('元')}}
				</view>
				
				<view class="go_buy">
					<button type="warn" class="mini-btn" @click="goGiftBag" style="border-radius:7px!important;line-height:96upx;height: 48px;background:#2c394d;font-size:18px;width: 100%;">
						{{__('立即购买')}}
					</button>
				</view>
			</view>
			...
			<view class="uni-banner giftbag" v-if="ProductInfo.gift_rows" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;width: 680upx;height:700upx;padding:26upx;">
				<label class="activity-title">{{ProductInfo.activity_item_row.activity_name}}</label>
				<scroll-view  class="scroll-view_H" scroll-x="true">
					<view class="scroll-view-item_H" v-for="(g_item,i) in ProductInfo.activity_item_row.activity_rule.item_rows">
						<view>
							<view hover-class="none" :data-pid="(g_item.item_id)" :data-state="(g_item.product_state_id)">
								<view class="product-l-img">
				...
				<block v-if="ProductInfo.activity_item_row.activity_rule.activity_join_mode == 1">
					<view class="gift-rule" v-for="(rule,r) in ProductInfo.activity_item_row.activity_rule.rule"> <!--按数量-->
			...
			<view class="uni-banner giftbag" v-if="ProductInfo.activity_item_row" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;width: 680upx;padding:26upx;">
				<label class="activity-title">{{ProductInfo.activity_item_row.activity_title}}</label>
				<scroll-view  class="scroll-view_H" scroll-x="true">
					<view class="scroll-view-item_H" v-if="ProductInfo.activity_item_row" v-for="(reduction_again_item,i) in ProductInfo.activity_item_row.reduction_item_data">
						<view>
							<view hover-class="none" :data-pid="(reduction_again_item.item_id)" :data-state="(reduction_again_item.product_state_id)">
								<view class="product-l-img">
				...
				<view class="gift-rule">
					<view v-if="ProductInfo.activity_item_row.reduction_rule" v-for="rule_items in ProductInfo.activity_item_row.reduction_rule">
			...
			<view class="uni-banner giftbag" v-if="ProductInfo.fold_rows" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;width: 680upx;padding:26upx;">
				<label class="activity-title">{{ProductInfo.activity_item_row.activity_title}}</label>
				<scroll-view  class="scroll-view_H" scroll-x="true">
					<view class="scroll-view-item_H" v-if="ProductInfo.fold_rows" v-for="(fold_rows_item,i) in ProductInfo.fold_rows">
						<view>
							<view hover-class="none" :data-pid="(fold_rows_item.item_id)" :data-state="(fold_rows_item.product_state_id)">
								<view class="product-l-img">
				...
				<view class="gift-rule">
					<view v-if="ProductInfo.activity_item_row.activity_rule.rule" v-for="rule_items in ProductInfo.activity_item_row.activity_rule.rule">
			...
			<view class="uni-banner giftbag" v-if="ProductInfo.reduction_again !== undefined" style="box-shadow: 2px 9px 11px #001;background:#FFFFFF;border-radius: 7px;width: 680upx;padding:26upx;">
				<label class="activity-title">{{ProductInfo.activity_item_row.activity_title}}</label>
				<scroll-view  class="scroll-view_H" scroll-x="true">
					<view class="scroll-view-item_H" v-for="(reduction_again_item,i) in ProductInfo.reduction_again.items">
						<view>
							<view hover-class="none" :data-pid="(reduction_again_item.item_id)" :data-state="(reduction_again_item.product_state_id)">
								<view class="product-l-img">
									<image style="width:100%;height:100%" :src="reduction_again_item.product_image" />
				...
				<view class="gift-rule">
					<view v-for="rule_items in ProductInfo.reduction_again.rule">
						满<span style="color: red;font-size: 16px;font-weight: bold;">{{rule_items.total}}</span>元，可省去<span style="color: red;font-size: 16px;font-weight: bold;">{{rule_items.max_num}}</span>元
					</view>
				</view>
	...
	export default {
		data() {
			return {
				...
				shareData: {
					shareText: 'JDMall商城系统，支持原生App、微信小程序，邀请你一起体验！',
					shareTitle: 'JDMall商城系统，支持原生App、微信小程序，邀请你一起体验！',
					href: "",
					image: '',
					price: 0
				},
		...
		metaInfo () {
			return {
			  meta: [
				{name: 'og:url', content: this.$.sprintf('%s/h5/pages/product/detail?pid=%d&uid=%d', this.Config.SiteUrl, this.proId, this.userInfo.user_id) },
		...
		methods: {
			...
			InitData: function(options) {
				...
				//that.notice.postNotificationName("RefreshProduct", !1)
					...
					success: function(data, status, msg, code) {
						if (200 == status) {
							...
							for (var n in data.item_row.product_spec) {
								...
								for (var r in data.item_row.product_spec[n].item) {
									...
									if (data.item_row.item_spec[n].item.id == data.item_row.product_spec[n].item[r].id) {
										data.item_row.SpecLst[n].svLst[r].IsChecked = !0;
									} else {
										data.item_row.SpecLst[n].svLst[r].IsChecked = !1;
									}
							...
							that.setData({
								...
								isCollection: data.is_favorite > 0 ? !0 : !1,
        ...
        */
			...
			addCart: function() {
				...
				that.$.request({
					...
					success: function(data, status, msg, code) {
						if (status == 200) {
							//that.notice.postNotificationName("RefreshProduct", !0);
...
<style lang="scss">
	...
	.dis {
		...
		width: 180upx;float:right;padding-top: 10upx;line-height: 74upx;
	}
	...
	.selled {
		display: flex;
		justify-content: flex-end;
		width: 180upx;float:right;line-height: 74upx;
	}
	...
	.m-panel-sp-sellabel-no {
		...
		border: 1upx solid #d5d5d5 !important;
		*/
	...
	.m-price-tip::after {
		position: absolute;
		content: "";
		border: solid 10upx $default-skin-bg;
...
.sgar {
  ...
  font-size: 20upx;
  background: #333959;
  padding: 0upx 15upx !important;
  border-radius: 5upx;
  display: inline-block !important;
  margin: 0upx 10upx 0upx 50upx !important;
}
