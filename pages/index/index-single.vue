<template>
	<view class="page" v-if="loadPageFlag" :style="{background:(BgConfig.BackgroundObj.type==1?BgConfig.BackgroundObj.bgColor:('url('+BgConfig.BackgroundObj.path+') no-repeat '+ BgConfig.BackgroundObj.pathColor))}">
		...
		<uni-nav-bar :leftFlag="!!Config.SUBSITE_ENABLE || !!Config.LANG_ENABLE" :rightFlag="rightFlag"  :statusBar="true" :fixed="true" :color="PageNav.window.navigationBarTextStyle" :backgroundColor="PageNav.window.navigationBarBackgroundColor" :background-color="PageNav.window.navigationBarBackgroundColor" right-icon="scan" right-width="120"  @click-left="showCity" @click-right="scan">
			<block slot="left">
			...
			<block slot="right">
				<view style="padding-right: 14upx;line-height: 80rpx;opacity: 0.6;position:relative;" @click.stop="$.gopage('/member/member/message')">
					<uni-icons type="chat" :color="PageNav.window.navigationBarTextStyle" size="24" :style="'color:' + PageNav.window.navigationBarTextStyle" />
					<view  class="msg-num" v-if="newMsgNum>0">{{newMsgNum}}</view>
				</view>
			</block>
		</uni-nav-bar>

		<diy :StoreInfo="ChatInfo" :BgConfig="BgConfig"  :PageContent="PageContent" :pageId="pageId" :PageNav="PageNav" :loadPageFlag="loadPageFlag" :hideDiy="hideDiy"></diy>
	...
	export default {
		...
		methods: {
			...
			initData: function(force_refresh) {
				...
				thatProm.TemplateKey = that.plantformInfo.plantform_template > 1000 ? 'shopdiy' : that.plantformInfo.plantform_template;
