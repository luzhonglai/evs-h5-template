	...
	<view>
		<view v-if="(CashData.isShowDistributionButton||isShowBtn)">
			...
			<view class="m-text-box" v-else>
				...
				<view>
					<view>{{__('成功邀请一位好友，可获积分奖励：')}}<label style="color:red;padding-left:10rpx;">{{plantformInfo.plantform_fx_gift_point}}</label>{{__('枚')}}, {{__('当好友产生消费时，再获得相应佣金奖励')}}</view>
				</view>
			</view>
	...
	export default {
		...
		onLoad: function(options) {
			...
			this.getPlantformInfo(function(site) {
				that.setData({
					StoreName: site.name,
					StoreLogo: site.logo,
					Level1Discount: that.plantformInfo.plantform_fx_cps_rate_1,
					Level2Discount: that.plantformInfo.plantform_fx_cps_rate_2,
		...
		methods: {
			...
			getPageInfo: function(e) {
				...
				// #ifdef APP-PLUS
				$href = that.$.sprintf('%s/h5/pages/index/index?uid=%d', that.Config.SiteUrl, e);
				// #endif
				
				// #ifdef MP-WEIXIN
				$href = that.$.sprintf('%s/pages/index/index?uid=%d', that.Config.SiteUrl, e);
				// #endif
            ...
            onShareBox: function(e) {
				...
				// #ifdef MP-WEIXIN
				$href = that.$.sprintf('%s/pages/index/index?uid=%d', that.Config.SiteUrl, this.userInfo.user_id);
				// #endif
