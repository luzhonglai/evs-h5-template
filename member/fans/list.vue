			...
			<view class="m-panel-bd">
				<view v-for="(item, index) in UserFans"  :key="index" class="m-media-box m-media-box-appmsg">
					...
					<view class="m-media-box-bd">
						<label class="m-media-box-title">{{item.user_nickname}}</label><label class="m-media-box-level" v-if="item.user_level_name">{{item.user_level_name}}</label><label class="m-media-box-num">{{__('收益：')}}<text>{{__('￥')}}{{Number(item.ugc_amount).toFixed(2)}}</text></label>
