<template>
	<view class="page">
		<scroll-view class='u-pa1' @scrolltolower="scrollbottom" scroll-y="true"
			...
			<view style='width:100%;height:38rpx;background:#333959'></view>
			<view style="background:#f7f7f7;position:relative">
				<view class="header">
					<view :class="member.IsEnabled?'header-top':'if_carh1'">
						<view style="width: 94%;margin: 0 auto;border-radius: 12px;"
							...
							<view style="width: 100%;display: flex;" :class="member.IsEnabled?'':'if_carh'">
								...
								<view class="grade"
									...
									<view style="margin: 2% 0 0 0;text-align: left;font-family: PingFangSC-Regular;font-size: 22rpx;color: #8f6846;">
										{{message.user_level_name||__('普通会员')}}
										<label style="float: right;margin-right: 20rpx;"  v-if="Config.PLANTFORM_FX_ENABLE"  @click="copyId(message.user_id)">{{__('推广码')}}: {{message.user_id}}    </label>
									</view>
								</view>
							</view>
							<view v-if="member.IsEnabled == true" class='card_vip' style="overflow: hidden;">
								<view class='show_card' data-index="index" style='position: relative;'>
									...
									<view v-if="member.IsEnabled && member.IsReceive && member.ReciveStatus == 1 && member.IsToWeixin "
										...
										<image src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/erweima.png'
						...
						<view v-if="member.IsEnabled && member.IsReceive && member.ReciveStatus == 1 && member.IsToWeixin "
							...
							<image style="width: 106px;height: 29px;border-radius: 0;" src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/cardp-bg.png'></image>
						</view>
					</view>
				</view>
				<view :class="member.IsEnabled?'header-bottom':'if_carh4'">
					...
					<navigator url="/member/cash/predeposit" class="growth" v-if="isShowMoney">
				...
				<view style='background:#fff'>
					...
					<view class="mission" style='padding:26rpx 0rpx' v-if="false">
						<view class="draw">
							<image style='width:12px;height:12px' src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/qiandao.png"></image>
							<text>{{__('签到领积分')}}</text>
						</view>
						<button style="margin-right: 0px;padding-left: 9rpx; padding-right: 9rpx; "  @click="click"
								:class="isSign?'color':'button'">{{__('去签到')}}
						</button>
					</view>
					<view class="mission" style='padding:26rpx 0rpx'>
						<view class="draw">
							<image style='width:12px;height:10px' src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/gouwu.png"></image>
							<text>{{__('购物领积分')}}</text>
						</view>
					...
					<view class="mission" style='padding:26rpx 0rpx'>
						<view class="draw">
							<image style='width:12px;height:12px' src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/pinglun.png"></image>
							<text>{{__('评价领积分')}}</text>
						</view>
					...
					<view class="alert" v-if="flag" style="z-index:1000">
						<image style="width:50rpx;height:50rpx"
							   src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/true.png"></image>
