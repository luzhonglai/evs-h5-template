<template>
	<view class="page">
		<view class="m-product-all">
			...
			<view :class="[tapindex==1?'':'hide']" style="font-size:28upx;position:absolute;top:66upx">
				...
				<view style="margin-top:20upx;background:#fff;overflow:hidden;">
					<view class="how">
						<text>{{__('如何获取')}}</text>
					</view>
			...
			<view :class="(tapindex==2?'':'hide')" style="font-size:28upx">
				<scroll-view v-if="(list.length>0)" scroll-y="true" class="m-orderlist" @scrolltolower="scrollbottom"  style="width:100%;height:100%;position:absolute;top:66upx">
					<view class="detail" v-for="(items, i) in list"  :key="i">
						<view class="detail-left">
							<text>{{items.exp_log_desc}}</text>
							<view class="time" style="margin-top:20upx">{{items.exp_log_time}}</view>
						</view>
...
<script>
	...
	export default {
		data: function() {
			return {
                list: [], tapindex: 1, page: 1, ispage: !0, flag: !0, Info: "", growthvalue: "", windowHeight: 0
			}
		},
		...
		onLoad: function(options) {
            ...
            // 等级规则
            that.$.request({
                url: this.Config.URL.user.lists_base_level,
                data: params,
            ...
            // 获取成长经验的途径
            that.$.request({
                url: this.Config.URL.user.listsExpRule,
                data: params,
                success: function (data, status, msg, code) {
                    if (200 == status) {
                        that.setData({
		...
		methods: {
            ...
            getMemberGrowthDetailList: function () {
                var that = this, params = { page: this.page, rows: 20};
                // 列出成长值
                that.$.request({
...
<style lang="scss">
	...
	.detail {padding: 24upx 30upx;position: relative;display: -webkit-box;display: -webkit-flex;display: flex;-webkit-box-align: center;-webkit-align-items: center;align-items: center;justify-content: space-between;background: #fff}
