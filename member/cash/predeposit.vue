<template>
    <view class="page">
        ...
        <view class="m-cells">
            ...
            <navigator url="/member/cash/rechargelist" class="m-cell m-cell-access" v-if="(isMemDist||isECashCard)">
                <view class="m-cell-bd">
                    <label>{{__('充值记录')}}</label>
                </view>
                <view class="m-cell-ft"></view>
        ...
        <view class="m-btn-box" v-if="(CashData.isShowDistributionButton&&isMemDist)">
            <navigator url="/member/fans/endorsement" class="u-btn btn-line-primary">{{__('我要代言')}}</navigator>
        </view>
    </view>
</template>
	...
	export default {
		...
		onLoad: function(options) {
			...
			*/
            ...
            // r.indexOf("ECashCard") > -1 ? this.setData({isECashCard: !0}) : this.setData({isECashCard: !1}),
            // r.indexOf("MemDist") > -1 ? this.setData({isMemDist: !0}) : this.setData({isMemDist: !1})
            // 系統配置是否启用分销、提现等系统
		},
		...
		methods: {
            ...
            showTip: function () {
                let that = this;
                that.$.confirm(that.__("只要你在店铺分享任何商品或者活动页面到微信，吸引到朋友点击并且进入店铺，TA即会成为你的一级粉丝，TA分享的一级粉丝会成为你的二级粉丝，一、二级粉丝支付购买的任何店铺内的商品，都会按照一定的计算方法算作你的奖金收益。 如果没有获得收益，主要可能有以下原因：- 你的朋友在之前已经成为其他人粉丝；- 系统判定该笔订单数据异常，收益被取消；- 自己点击了自己分享的商品链接。"))
            },
            "goto": function () {
                let that = this;
...
<style lang="scss">
    ...
    .topbg {background-color: $default-skin-bg;padding-bottom: 20upx;}
    .topinfo {display: flex;width: 100%;flex-flow: column;padding-top:40upx;justify-content: center;align-items: center;color: white;font-size: 14px;position: relative;}
    /* .topinfo label {font-size: 56px;} */
    .m-t-text{float: right;position: relative;margin-right: 40upx;}
    .m-t-text::after {content:" ";display:inline-block;height:12upx;width:12upx;border-width:4upx 4upx 0 0;border-color:#fff;border-style:solid;-webkit-transform:matrix(0.71, 0.71, -0.71, 0.71, 0, 0);transform:matrix(0.71, 0.71, -0.71, 0.71, 0, 0);position:relative;position:absolute;top: 50%;margin-top: -6upx;}

    .menu_list{display: -webkit-box;margin-top: 40upx;}
    .menu_item{color: white;float: left;-webkit-box-flex: 1;box-sizing: border-box;text-align: center;
        position: relative;}
    .menu_item text{font-size: 14px;}
    .menu_item:nth-child(1):before {content: "";border-right: 1upx solid white;height: 50upx;position: absolute;right: 0;bottom: 0;}
    .m-cells{font-size: 28upx;background-color: #fff;}
    .m-cell-bd{color: #333;}
    .m-text-box{font-size: 24upx;box-sizing: border-box;padding: 20upx;color: #888;line-height: 40upx;}
    .m-text-box label{padding-right: 10upx;}
    .m-cell-bd{line-height: 50upx;}
    /* .m-btn-box{padding-bottom: 10upx;} */
    .m-btn-box-two{
        display: flex;
