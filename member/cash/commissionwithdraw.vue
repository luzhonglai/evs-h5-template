	...
	<view class="page">
		...
		<view :class="(tapindex==1?'':'hide')">
			...
			<view class="m-cells m-cells-form">
				<view class="m-cell">
					...
					<view class="m-cell-bd">
						<input class="u-input" type="number" :value="(Price)" :placeholder="__('输入提现金额(需要大于:') + withdraw_min_amount + ')'" maxlength="6" @input="onInputPrice" />
					</view>
				...
				<view class="m-cell">
					...
					<view class="m-cell-bd">
						<input class="u-input" type="text" :value="(BankName)" placeholder="如：上海农业银行七宝支行" @input="inputbankName" />
					</view>
			...
			<view class="m-text-box">
				...
				<view>
					<view>{{__('粉丝在店铺内订单支付后你将会获得佣金收益，收益在订单支付后')}}
						<label style="color:red">{{ValidDays}}</label>{{__('天可提现。订单若发生退款你将不会收到佣金收益。')}}
					</view>
				</view>
			</view>
			<!-- <view class="m-btn-box m-btn-box-two">
              <button class="u-btn u-btn-default" @click="submitdata">申请提现</button>
              <navigator url="/member/cash/recharge" class="u-btn u-btn-default">转入余额</navigator>
            </view> -->
			<view class="btn_box">
				<button class="u-btn u-btn-default" @click="submitdata">{{__('申请提现')}}</button>
			</view>
			<view class="btn_box" v-if="(isECashCard)">
		...
		<view :class="(tapindex==2?'':'hide')">
			...
			<view class="m-cells m-cells-form">
				<view class="m-cell">
					...
					<view class="m-cell-bd">
						<input class="u-input" type="number" :value="(Price)" :placeholder="__('输入提现金额(需要大于:') + withdraw_min_amount + ')'" maxlength="6" @input="onInputPrice" disabled="true" />
					</view>
					<view v-if="(!isPrice)" class="m-cell-ft">
						<icon class="m-icon-warn" type="warn"></icon>
					</view>
				...
				<view class="m-cell">
					...
					<view class="m-cell-bd">
						 <picker @change="bindPickerChange" :value="index" :range="banklists" range-key="bank_name">
						           <view class="uni-input">{{banklists[index] ? banklists[index]['bank_name'] : __('请选择收款方式')}}</view>
						  </picker>
					</view>
					<view v-if="(!isbankName)" class="m-cell-ft">
						<icon class="m-icon-warn" type="warn"></icon>
			...
			<view class="m-text-box">
				...
				<view>
					<view>
						{{__('提示:每月的')}} <label style="color:red;padding-left:10rpx;">{{withdraw_monthday}}</label> {{__('可以申请提现，最低提现额度')}}<label style="color:red;padding-left:10rpx;">{{withdraw_min_amount}}</label>，{{__('商家营收余额需要买家确认收货后')}} <label style="color:red;padding-left:10rpx;">{{withdraw_received_day}}</label> {{__('天才可以申请提现')}}。
					</view>
				</view>
			</view>
			<view class="btn_box">
			...
			<view class="btn_box">
				<button class="u-btn u-btn-default" @click="listWithdraw">{{__('提现记录')}}</button>
			</view>
		</view>
		...
		<uni-popup ref="popup" type="center">
			<view class="uni-banner" style="background:#FFFFFF;"  v-if="loaded">
				<view :style="'height: ' + h + 'px; width:700uxp;padding:50upx;'">
...
<script>
	...
	export default {
			...
			that.$.request({
				...
				success: function(data, status, msg, code) {
					if (status == 200) {
						that.setData({
							TotalPrice: 0, // 佣金
							canUseCardCashAmount: data.user_commission, //佣金
							Price: data.user_commission, //佣金
							ValidDays: '',
		...
		methods: {
			...
			submitdata: function() {
				let that = this;
				this.CashType == 0 ? that.$.isNull(this.Price) ? this.setData({
						isPrice: !1
					}) : parseInt(this.Price) < 100 || this.Price % 100 > 0 ? (this.setData({
						isPrice: !1
					}), that.$.alert(that.__("亲~提现金额必须是100的整数倍才能提现哟！"))) : this.setData({
						isPrice: !0
					}) : this.setData({
						isPrice: !0
				...
				if (this.isPrice && this.isWXcode && this.isPhone && this.isName && this.isbankAccount && this.isalipayAccount &&
					this.isbankName) {
					var params = {
					...
					// todo 佣金提现
					that.$.request({
					    url: this.Config.URL.fx.doWithdrawMoney,
						method:"POST",
					    data: params,
					    success: function (data, status, msg, code) {
					        if (200 == status) {
					            (that.$.showModal(that.__("已申请提现！")), setTimeout(function () {
			...
			ApplyToCash: function() {
				...
				if (this.CashType == 0)
				{
					...
					{
						if (parseInt(this.Price) < this.withdraw_min_amount)
						{
							this.setData({
								isPrice: !1
							}), that.$.alert(that.__("亲~提现金额必须大于") + this.withdraw_min_amount + that.__("才能提现哟！"))
						}
						else
						{
							this.setData({
			...
			listWithdraw: function() {
				let that = this;
				that.$.gopage("/member/cash/predepositapplylist");
			},
			move: function() {
				let that = this;
				this.CashType == 0 ? that.$.isNull(this.Price) ? this.setData({
				...
				if (this.isPrice) {
					var e = {
						...
						totalBonusesCash: this.Price
					};
					// todo 佣金转余额
					that.$.xsr(that.$.makeUrl(userapi.CommisionToECash, e), function(e) {
						e.Code == 0 ? (that.$.alert(that.__("已转入余额！")), setTimeout(function() {
								that.$.navigateBack(1, function() {})
							},
							2e3)) : that.$.alert(e.Msg)
					})
				}
			},
