<template>
	<view >
		...
		<view class="m-tips" style="margin:0upx 0;" v-if="titleText">
		...
		<view class="m-tips" style="margin:0upx 0;color:#000000" v-if="titleText">
			<!-- table切换start -->
			<scroll-view class="scroll-view_S" scroll-x show-scrollbar="false" :style="{backgroundColor:bgColor}">
				<view class="scroll-view-item_S"  >
					<label :class="['m-navbar-item', tapindex==-1?'m-navbar-item-on':'']" style="padding:0;" :data-index="-1" :data-id="0" @click="setTab">{{__('推荐')}}</label>
				</view>
				<view class="scroll-view-item_S"  v-for="(items,index) in store_category_lists" :key="index">
					<label :class="['m-navbar-item', tapindex==index?'m-navbar-item-on':'']" style="padding:0;" :data-index="index" :data-id="items.store_category_id" @click="setTab">{{items.store_category_name}}</label>
				</view>
			</scroll-view>
			<!-- table切换end -->
			<!-- 全部订单start -->
		...
		<view v-if="loadComplete">
			<block v-if="true">
				<HMfilterDropdown :filterData="filterData" :defaultSelected ="filterDropdownValue" :updateMenuName="true" :scrollTop="hight" @confirm="confirm" v-if="isShowFilter"  :style="isFilterTop0 ? '' : 'position: inherit;'"></HMfilterDropdown>
					...
					<scroll-view class="order-list" scroll-y="true" @scrolltolower="scrollbottom" :style="isShowFilter&&isFilterTop0 ? 'padding-top:100rpx' :'padding-top:20rpx'">
						<view class="list">
							<view class="row"  v-for="(item,index) in Info" :key="index" >
							 ...
							 :data-name="(item.store_name)" :data-lat="(item.store_latitude)" :data-lng="(item.store_longitude)"><image style="width:20upx;height:26upx;line-height: 26rpx;" src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/location.png'></image><label class="u-font" style="line-height: 26rpx;">{{item.distance}}</label></label>
										</view>
								...
								<view class="product-item" v-if="(item.products && item.products.length > 0)">
									<navigator :url="'/pages/product/detail?pid=' + (val.item_id)" class="item-s" v-for="(val,ii) in item.products" :key="ii">
										<view class="p-image">
							...
							<view class="row"  v-for="(item,index) in Info" :key="index" >
								<navigator class="order-info" :url="'/pagesub/index/store?store_id=' + (item.store_id)">
									...
									<view class="right">
										<view class="name">
											<label class="store-name">{{item.store_name}}</label>
											<label style="display: flex;"><image style="width:20rpx;height:26rpx;line-height: 26rpx;" src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/location.png'></image><label class="u-font" style="line-height: 26rpx;">{{item.distance}}</label></label>
										</view>
										<view class='flex-info-box'  style="display: flex;align-items:center;">
											<image src="../static/images/xing.png" style="width:26rpx;height:26rpx;"></image>
											<label class="u-font evaluation-rate">{{sprintf(__('%s分'), item.store_evaluation_rate)}}</label>
		...
		<view v-if="isdata && issshow" class='open_ba'>
			<view style="width: 200rpx;margin: 71px auto 25px auto;wxcs_style_margin: 143rpx auto 50rpx auto;auto: 50rpx auto;">
				<image src='https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/img/ditulogo.png' style="width: 200rpx;height: 200rpx;"></image>
			</view>
