	...
	<view v-if="commonTplData !== null">
		...
		<view class="content" :style="{height:(commonTplData.windowHeight) + 'px'}">
			<scroll-view class="category" scroll-y="true" :style="{height:(commonTplData.windowHeight) + 'px'}">
				<view :class="['category-item', commonTplData.cid==item.store_product_cat_id?'category-item-select':'']" v-for="(item, i) in commonTplData.smallCategory"
				 :key="i" @click="ckallPD6" :data-id="(item.store_product_cat_id)" :data-name="(item.name)">
					{{item.name}}
					<view class="car-num" style="top: unset;margin-top: -84upx;" v-if="(item.user_cart_quantity !== undefined && item.user_cart_quantity > 0)">{{item.user_cart_quantity ? item.user_cart_quantity : ""}}</view>
				 </view>
			...
			<scroll-view class="body" scroll-y="true" v-if="(commonTplData.pdlist.length > 0)" @scrolltolower="scrollbottom" :style="{height:commonTplData.windowHeight + 'px'}">
				<view class="body-title" v-if="commonTplData.categoryname">{{commonTplData.categoryname}}</view>
				 ...
				 hover-class="none">
					...
					<view class="body-item-info">
						...
						<view class="item-info-price">
							...
							<view class="item-num-box" style="bottom:0;right:0;font-weight: normal;height:50upx;">
								...
								<block v-else>
									<block v-if="(!item.HasManualSku)">
										<view @click.stop="subcontent" :data-num="(item.user_cart_quantity)" :data-stock="(item.Stock)" :data-spuid="(item.id ? item.id : 0)"
										 ...
										 v-if="(item.user_cart_quantity>0)" class="iconfont icon-jian " style="font-size:50upx;"></view>
										...
										<view @click.stop="addcontent" :data-num="(item.user_cart_quantity)" :data-stock="(item.Stock)" :data-spuid="(item.id ? item.id : 0)"
										 ...
										 class="iconfont icon-add " style="font-size:50upx;position:relative;bottom:5upx;"></view>
									</block>
									<label class="item-num-choose" @click.stop="InitProduct" :data-item_id="(item.item_id)" :data-index="(index)"
									 v-else>{{__('选规格')}}
										<view class="car-num" v-if="(item.user_cart_quantity>0)" style="right:-10upx;top:-15upx;">{{item.user_cart_quantity>99?'99+':item.user_cart_quantity}}</view>
									</label>
								</block>
							...
							<view class="window-scroll-item" v-if="(item.cart_select)">
								...
								<view class="item-num-box">
									<view @click.stop="sub" :data-num="item.cart_quantity" :data-stock="item.item_quantity"
									 :data-cid="item.cart_id" :data-spuid="item.product_id" :data-skuid="item.item_id" class="iconfont icon-jian icon" style="font-size:45upx;"></view>
									...
									<view @click.stop="add" :data-num="item.cart_quantity" :data-stock="item.item_quantity"
									 :data-cid="item.cart_id" :data-spuid="item.product_id" :data-skuid="item.item_id" class="iconfont icon-add icon" style="font-size:45upx;position:relative;bottom:5upx;"></view>
		...
		<view class="msk1" v-if="(commonTplData.addCar)" style="z-index:20;" @click.stop="closeaddcar">
			<view class="m-sp-box" @click.stop="innertouch">
				...
				<view class="m-sp-bottom">
					...
					<view class="item-num-box" style="bottom:0;right:30upx;font-weight: normal;height:100upx;">
						<block>
							<view @click.stop="subcontentsp" v-if="(commonTplData.count>0)" :data-stock="(commonTplData.ProductInfo.item_quantity)"
							 ...
							 :data-skuid="(commonTplData.ProductInfo.item_id)" class="iconfont icon-jian " style="font-size:50upx;"></view>
	...
	export default {
		...
		methods: {
			...
			GetPlistTakeAway: function() {
				...
				that.$set(that.commonTplData, 'flag', !1)
					...
					success: function(data, status, msg, code) {
						...
						if (data.items.length > 0) {
							if (that.commonTplData.post.curpage == 1 && data.items.length < 10) {
								...
								that.$set(that.commonTplData, 'flag', !1);
								that.$set(that.commonTplData, 'ispage', !1);
							} else {
								that.$set(that.commonTplData, 'flag', !0);
								that.$set(that.commonTplData, 'ispage', !0);
							}
			...
			cancelwindow: function() {
				let that = this;
				that.$set(that.commonTplData, 'click6', !0);
			},
			doNothing: function() {
			},
			...
			requestCartList: function() {
				...
				return new Promise((res) => {
					...
					that.$.request({
						...
						loading: false,
						success: function(data, status, msg, code) {
							that.$set(that.commonTplData, 'isdata', data.total_quantity > 0 ? !0 : !1);
							if (data.items.length > 0)
							{
				...
				$.request({
					...
					success: function(data, status) {
						if (status != 200) {
							...
							that.$set(that.commonTplData, 'ProductInfo', data.item_row);
							that.$set(that.commonTplData, 'addCar', !0);
			...
			closeaddcar: function() {
				let that = this;
				that.$set(that.commonTplData, 'addCar', !1);
			},
							...
							if (i[s].type == 2) {
								if (u) {
									if (!!$.isNull(t.formdate)) {
										return $.confirm(i[s].placeholderText), c = !1, !1;
									}
									o = t.formdate, c = !0
								} else {
									c = !0;
