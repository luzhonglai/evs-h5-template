<template>
	...
	-->
		...
		<view class="emojiList" :style="[{'transform': 'translateX(' + emojiMarginLeft + 'px)'},{'width':emojiWidth+'px'},{'transition': transition }]"
		 @touchstart="gtouchstart($event)" @touchmove="gtouchmove($event)" @touchend="gtouchend($event)">
			<view class="item">
				<view v-for="(item1,index1) in beforeList" :key="index1">
					<image v-if="item1.url!=''" :src="items[groupIndex -1].minEmoji && index1===beforeList.length -1  ? item1.url : items[groupIndex -1].emojiPath + item1.url "
					 class="emojiImg" :class="[{ 'minImg' : items[groupIndex-1].minEmoji },{'bigImg' : !items[groupIndex-1].minEmoji }]" />
			...
			<view class="item" v-for="(item3,index3) in items[groupIndex].emojiList" :key="index3">
				<view v-for="(em4,index4) in item3" :key="index4">
					<image :src="items[groupIndex].minEmoji && index4 === item3.length-1 ? em4.url : items[groupIndex].emojiPath + em4.url "
					 class="emojiImg" :class="[{ 'minImg' : items[groupIndex].minEmoji },{'bigImg' : !items[groupIndex].minEmoji }]"
			...
			<view class="item">
				<view v-for="(item5,index5) in nextList" :key="index5">
					<image v-if="item5.url!=''" :src="items[groupIndex +1].minEmoji && index5 === nextList.length -1  ? item5.url : items[groupIndex +1].emojiPath + item5.url "
					 class="emojiImg" :class="[{ 'minImg' : items[groupIndex+1].minEmoji },{'bigImg' : !items[groupIndex+1].minEmoji }]" />
		...
		<block v-if="false">
			<image src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/im/img/tab/add2.png" @tap="addEmojiPackage" style="float:left;margin-top: -64px; margin-left: 12px; width: 32px;height: 32px;" />
			<scroll-view class="scroll-view_H" scroll-x="true" @scroll="scroll" scroll-left="120">

				<view class="scroll-view-item_H" v-for="(emoji,index) in items" :key="index" v-if="index!==0 && index!==items.length-1">
					<image style="width: 32px;height: 32px;" @tap="selectTab(1,index)" :src="emoji.emojiPath + emoji.emojiList[0][0].url"
					 :class="{'activeEmojiTab':activeEmojiTab ===index}" />
				</view>

			</scroll-view>

			<image src="https://e-mall-1316691179.cos.ap-shanghai.myqcloud.com/mall/appicon/im/img/tab/setting.png" @tap="settingEmoji" style="float:left;margin-top: -64px; margin-left: 320px; width: 32px;height: 32px;">
		</block>
